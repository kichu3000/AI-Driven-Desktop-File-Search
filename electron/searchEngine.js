...
export async function searchFiles(filters){ //"filters" is the json return by the AI api.
        ...
        try {
            ...
            for(const item of items){
                ...
                if (item.isDirectory()) {
                    ...
                    const isExcluded = EXCLUDED_FOLDERS.some(
                        folder => folder.toLowerCase() === folderName
                    );

                    if (isExcluded) {
                        console.log("Excluded folder:", fullPath);
                        continue;
                    }
                }
                if(item.isDirectory()){
    ...
    const isEffectivelyEmpty =
        (!normalizedQuery.includeFileTypes.length) &&
        (!normalizedQuery.excludeFileTypes.length) &&
        (!normalizedQuery.filenameKeywords.length) &&
        (!normalizedQuery.contentKeywords.length) &&
        normalizedQuery.minSizeMB === null &&
        normalizedQuery.maxSizeMB === null &&
        normalizedQuery.createdFrom === null &&
        normalizedQuery.createdTo === null &&
        normalizedQuery.modifiedFrom === null &&
        normalizedQuery.modifiedTo === null &&
        !normalizedQuery.drive &&
        !normalizedQuery.folder;

    if (isEffectivelyEmpty) {
        console.log("Blocked: empty search would scan full disk");
        return [];
    }

    const drivesToScan = normalizedQuery.drive
        ? [normalizedQuery.drive + "\\"]
        : ["C:\\", "D:\\", "E:\\"];
    ...
    for (const drive of drivesToScan) {
        try {
            ...
            const res = await scanDirectory(drive, normalizedQuery,[]);
            allResults.push(...res);

        } catch (err) {
            console.log("Drive failed:", drive);
            continue;
        }
    }

    const sortedResults = sortResults(allResults, normalizedQuery);
